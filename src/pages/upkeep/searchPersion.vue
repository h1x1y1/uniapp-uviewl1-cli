...
<script>
	import { getPersoin, setPersoin,DelWorkUser,GetWorkUser } from './api'
	export default {
		data() {
			return {
				value: '',// 顶部搜索框里面的内容
		...
		onLoad(e) {
			this.McGuid = decodeURIComponent(e.McGuid)
			this.MTASSMCGuid = decodeURIComponent(e.MTASSMCGuid)
			this.MachineGuid = decodeURIComponent(e.MachineGuid)
			this.getPersoin()
