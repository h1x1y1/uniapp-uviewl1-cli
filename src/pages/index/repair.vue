	...
	<view class="container pl15 pr15">
		...
		<view class="" style="width: 690upx;">
			...
			<view v-if="listData.length > 0" v-for="(item, index) in listData" :key="index"
				...
				<!-- <view class="item flex align-items-center" style="height: 40upx;">
					<u-icon size="11" name="/static/item2/icon02.png"></u-icon>
					<text class="pl3">设备名称：</text>
					<text>张三</text>
				</view> -->
				<view class="item flex align-items-center" style="height: 40upx;">
				...
				<view class="item flex align-items-center" style="height: 40upx;">
					...
					<text class="flex-1">{{item.StartTime}}</text>
				</view>
				<view class="item flex align-items-center" style="height: 40upx;">
					<u-icon size="11" name="/static/item2/icon03.png"></u-icon>
					<text class="pl3">报修备注：</text>
					<text class="flex-1">{{item.Mark}}</text>
					<!-- <u--textarea v-model="value1" placeholder="请填写详细地址"></u--textarea> -->
				</view>
				<view v-if="btnStatus" class="absolute flex" style="bottom: 14upx; right: 17upx;">
					<!-- <view class="br20 fs12 color-white flex align-items-center flex-center mr15"
						style="background-color: #9771FF;height: 55upx; width: 126upx;"
						@click="navTo('/pages/my/select')">
						查看详情
					</view> -->
					<view class="br20 fs12 color-white flex align-items-center flex-center"
						style="background-color: #0CE283;height: 55upx; width: 126upx;" @click="getRepir(item)">
						接单维修
					</view>
				</view>
...
<script>
	...
	export default {
		...
		methods: {
			// 页面初始化
			init() {
				this.GetMachineMaintainItems();
			},
			// 盘点列表
			GetMachineMaintainItems(QRCode) {
				console.log(QRCode)
				let that = this;
				uni.showLoading({
					title: 'loading...'
			...
			scanCode() {
				...
				// 	success: function(res) {
						...
						that.GetMachineMaintainItems(newCode)
						return false
			...
			getRepir(item) {
				...
				let params = {}
				params.MachineGuid = item.MachineGuid
				params.Status = 1
				...
				params.Mark = item.Mark
				params.SupplierGuid = ''
				params.Distributioned_UserGuid = this.userInfo.UserGuid
				params.RepairType = 1
				params.Distribution_Type = 1

				SetMachineRepair(params).then(({
					data: {
						code,
						data,
						msg
