	...
	<view class="container">
		...
		<view class="li-login-form">
			<u-form class="li-input" :model="form" ref="uForm" :error-type="errorType">
				...
				<u-form-item :leftIconStyle="{color: '#888', fontSize: '34rpx', width:'34rpx', height:'34rpx'}"
					left-icon="/static/login/yqm.png" prop="company_code">
					<u-input placeholder="请输入公司邀请码" placeholderStyle="placeclass" maxlength="6"
						v-model="form.company_code" type="password"></u-input>
				</u-form-item>
			</u-form>
			<!-- 服务条款 -->
...
<script>
	...
	export default {
		...
		methods: {
			...
			submit() {
				...
				this.$refs.uForm.validate(valid => {
					if (valid) {
						...
						that.goRegist(that.form.email, that.form.code, that.form.password, that.form
							.repeat_password,
							that.form.company_code, );
			...
			// 注册
			goRegist(email, code, password, repeat_password, company_code) {
				let that = this
				let params = {
					email,
					code,
				...
				doRegisterByEmail(params).then(res => {
					console.log(res);
					if (res.data.code === 1) {
						that.$refs.uToast.show({
							title: '正在跳转登录',
